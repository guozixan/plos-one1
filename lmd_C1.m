...
for var = 1:3
    ...
    % 创建图例
    legend_labels = cell(length(C1_values), 1);
    for i = 1:length(C1_values)
        legend_labels{i} = ['C1=' num2str(C1_values(i))];
    end
...
% 保存图像（根据实际情况修改路径）
print(gcf, 'C:\Users\xin\Desktop\学业\12应急物流\matlab\图片\C1_xyz', '-dmeta', '-r600');